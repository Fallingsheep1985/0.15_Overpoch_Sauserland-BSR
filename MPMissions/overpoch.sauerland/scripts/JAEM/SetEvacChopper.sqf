...
_magazinesPlayer = weapons player;
...
//If not exit script
_hasBriefcase = {_x == "ItemRadio"} count _magazinesPlayer;
if (_hasBriefcase < evac_chopperPrice) then {
	if (playerHasEvacField) then {
		systemChat(format["Making a Evac-Chopper requires a Radio You dont have it - Sorry!", evac_chopperPrice]);
	} else {
		systemChat(format["Making a Evac-Chopper requires a Radio You dont have it - Sorry!", evac_chopperPrice]);
	};
	s_player_changeEvacChopper = -1;
...
if (_hasBriefcase < evac_chopperPrice) then {
	...
	[player, "ItemRadio", evac_chopperPrice] call BIS_fnc_invRemove;
	if (playerHasEvacField) then {
		systemChat(format["The Evac-Chopper has been set to your Radio Frequency!!!", evac_chopperPrice]);
	} else {
		systemChat(format["The Evac-Chopper has been set to your Radio Frequency!!!", evac_chopperPrice]);
	};
};
